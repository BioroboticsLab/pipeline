...
namespace heyho {
	...
	F convex_poly(F f, B boundaries, IT begin, IT end, int line_type)
	{
		...
		pixel_fwd_it_t side_1(ring_fwd_it_t(begin, end, min_y_it, max_y_it), static_cast<connectivity>(line_type));
		pixel_rev_it_t side_2(ring_rev_it_t(begin, end, min_y_it, max_y_it), static_cast<connectivity>(line_type));
	...
	inline F convex_poly(F f, B boundaries, cv::InputArray points, int line_type)
	{
