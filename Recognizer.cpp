...
void Recognizer::detectXieEllipse(Tag &tag) {
	...
	for (auto mit = cannyImage.begin<unsigned char>(), end = cannyImage.end<
			unsigned char>(); mit != end; ++mit) {
		if (*mit.ptr == 255) {
	...
	for (size_t lc1 = 0; lc1 < epsize; ++lc1) {
		...
		for (size_t lc2 = 0; lc2 < epsize; ++lc2) {
			...
			if ((lc2 > lc1) && ((p1x != p2x) || (p1y != p2y))) {
				...
				if (dist > recognizer_min_major
						&& dist < recognizer_max_major) {
					...
					for (size_t lc3 = 0; lc3 < epsize; ++lc3) {
						...
						if (((px != p2x) || (py != p2y))
								&& ((px != p1x) || (py != p1y))) {
							const double d = pointDistance(pxf, pyf, centerx,
									centery);
							if (d <= a && d > recognizer_min_minor / 2.0) {
								// (7) estimate the half length of the minor axis (b)
								const double f = pointDistanceNoSqrt(pxf, pyf,
										p2xf, p2yf);
								const double costau = (a * a + d * d - f)
										/ (2 * a * d);
								const double tau = acos(costau);
								const double sintau = sin(tau);
								const double b = (a * d * sintau)
										/ sqrt(
												((a * a)
														- (d * d * costau
																* costau)));
								...
								if (b2 <= recognizer_max_minor
										&& b2 >= recognizer_min_minor) {
									accu[cvRound(b) - 1] += 1;
								}
							}
						}
					...
					const auto max_it = std::max_element(accu.begin(),
							accu.end());
					...
					if (vote_minor >= _settings.get_threshold_edge_pixels()) {
						...
						const cv::Point2i cen(cvRound(centerx),
								cvRound(centery));
						...
						vote_minor =
								static_cast<int>(vote_minor * (50 * n / j));

						if (candidates.size() == 0) {
							candidates.emplace_back(vote_minor, cen, axis,
									angle, tag.getBox().size());
							if (vote_minor
									>= _settings.get_threshold_best_vote()) {
								goto foundEllipse;
							}
						}
						for (size_t idx = 0; idx < candidates.size(); idx++) {
							...
							if (std::abs(ell.getCen().x - cen.x) < 8
									...
									std::abs(ell.getAngle() - angle)
											< (180.0 * ell.getAxis().height)
													/ ell.getAxis().width) {
...
void Recognizer::visualizeEllipse(Tag const& tag, Ellipse const& ell,
		std::string const& title) {
	cv::Mat subroiTest = tag.getOrigSubImage().clone();
	ellipse(subroiTest, ell.getCen(), ell.getAxis(), ell.getAngle(), 0, 360,
			cv::Scalar(0, 0, 255));
	std::string text = "Score " + std::to_string(ell.getVote());
	cv::putText(subroiTest, text, cv::Point(10, 30),
			cv::FONT_HERSHEY_COMPLEX_SMALL, 0.7, cv::Scalar(0, 255, 0));
...
int Recognizer::calcScore(Ellipse ell, cv::Mat canny){
